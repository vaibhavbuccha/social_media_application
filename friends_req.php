    ...
    <div id="add">
        ...
        <div>
            <span><button onclick="back()" class="btn btn-outline-dark"><i class="fa fa-mail-reply" style="font-size:24px"></i></button></span>
                <span style="background:black;color:white;padding:10px;border: 1px doubled white;margin-left:180px;border-radius:20px;" align="center">Friend Requests</span>
        </div>

        <?php
        $user = $_SESSION['user'];
        $req_select = "SELECT * FROM friend_req WHERE reciver_name='$user' AND status='pending' ";
        $req_get = mysqli_query($con,$req_select);
        // print_r($req_get);
        while($row2 = mysqli_fetch_assoc($req_get)){
